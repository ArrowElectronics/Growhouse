...
static unsigned long ledCount = 0;

/* Function to test the BLE write */
void *blegatt_write_test(void *vargp);
struct client {
	int fd;
	struct bt_att *att;
	struct gatt_db *db;
...
static void ready_cb(bool success, uint8_t att_ecode, void *user_data)
{
	...
	/* Create the test thread that will send the write attribute request continously */
	pthread_create(&thread_id, NULL, blegatt_write_test, (void *)cli);
...
{
	//int i;
	static int packet_pre = 0;
	static int packet_cur = 0;
	static int packet_lost = 0;
	...
	printf("%d", value[0]);
	packet_cur = value[0];
	if((packet_cur - packet_pre > 0) && packet_cur - packet_pre != 1)
	{
	...
	if(packet_cur >= 255)
	{
		printf("Test stop, PER:%d, packet lost:%d\r\n", packet_lost, packet_lost);
		packet_pre = 0;
		packet_lost = 0;
	}
...
void *blegatt_write_test(void *vargp)
{
	struct client *cli = (struct client *) vargp;
	char buf[32] = {0};

	printf("Test thread for write gatt characteristics started...\n");
	cmd_register_notify(cli, "0x0011");
	while(1)
	{
		if(ledCount >= 255)
		{
		...
		snprintf(buf, 32, "0x0011 0x%lx", ledCount);

		/* Write the BLE characteritics for BLE write test */
		cmd_write_value(cli, buf);

		/* Delay between two packet write */
		usleep(200000);
	}
    return NULL;
}
