/**
 * @file rtos/generic/rtos-sleep-manager.c
...
void sleepManagerSleep(uint32_t durationMs)
{
  ...
  if ((durationMs > MIN_DEEP_SLEEP_DURATION_MS)
      && (dsBlockCount == 0)) {
    {
      ...
      halPowerDown();
#if RTOS_CONTROL_GPIO_WAKE
      halSleepWithOptions(SLEEPMODE_WAKETIMER, sleepGpioWakeBitMask);
#else
...
#if RTOS_CONTROL_GPIO_WAKE
    halSleepWithOptions(SLEEPMODE_IDLE, WAKE_MASK_INVALID);
#else
