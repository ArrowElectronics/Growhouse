...
#include "../plugin/ota-server/ota-server-dynamic-block-period.h"
...
#if defined(EMBER_AF_PLUGIN_OTA_SERVER_DYNAMIC_MIN_BLOCK_PERIOD_SUPPORT)
      if (clusterId == ZCL_OTA_BOOTLOAD_CLUSTER_ID) {
        emAfOtaServerReadAttributesResponseCallback(cmd->source,
                                                    message + msgIndex,
                                                    msgLen - msgIndex);
      }
