...
#define NETWORK_ADDRESS_RESPONSE_NODE_ID_OFFSET 10
                                                  ...
                                                  uint16_t length)
{
  ...
  if (i != DEVICE_QUEUE_NULL) {
    ...
    /* For LED node we are getting Six endPoints, So rather than registering Six
       end points we will register only one endpoint */
    if(message[ACTIVE_ENDPOINT_RESPONSE_COUNT_OFFSET] == 6)
      message[ACTIVE_ENDPOINT_RESPONSE_COUNT_OFFSET] = 1;
                                                ...
                                                uint16_t length)
{
  ...
  EmberNodeId remoteDeviceNodeId;
  ...
  switch (apsFrame->clusterId) {
    ...
    case NETWORK_ADDRESS_RESPONSE:
      emberAfCorePrintln("new device line %d", __LINE__);
      remoteDeviceNodeId = emberFetchLowHighInt16u(message + NETWORK_ADDRESS_RESPONSE_NODE_ID_OFFSET);
      emberAfCorePrintln("Remote Device node %2x", remoteDeviceNodeId);
      emberAfDeviceTableNewDeviceJoinHandler(remoteDeviceNodeId,
