...
class GroupConfigurationViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        groupNameLabel.text = viewModel.group.name
        groupIdLabel.text = viewModel.group.applicationKey.hexEncodedString()
        networkNameLabel.text = viewModel.services.networkManager.network(forNetworkIndex: viewModel.group.networkIndex)!.name
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == Storyboard.Network.Segue.editGroupName {
