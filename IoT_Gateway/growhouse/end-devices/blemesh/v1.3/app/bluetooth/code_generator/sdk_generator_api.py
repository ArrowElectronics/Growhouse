...
def sanityCheckConfiguration(codegenContext, sampleApp=None):
  ...
  for configModule in codegenContext:
    if "sampleApps" in configModule:
      for sa in configModule["sampleApps"]:
        if sa["id"] in sas:
          return generatorError("Error: duplicate SA: '%s'!" \
                                % sa["id"])
        sas.append(sa["id"])
        ...
        if "groups" in cgm:
          ...
          for cgmg in cgm["groups"]:
            if cgmg["id"] in cgmgIds:
              return generatorError("Error: duplicate CGMG: '%s' in CGM: '%s'!" \
                                    % (cgmg["id"], cgm["id"]))
        ...
        if not sampleApp or sa["id"] == sampleApp:
          ...
          for rcgm in rcgms:
            ...
            for rcgmg in rcgmgs:
              ...
              if not cgmgs:
                return generatorError("Error: '%s' CGMG is required by '%s' CGM of '%s' SA, but not defined!" \
  ...
  for cgKey in cgKeys:
    ...
    if not foundKey:
      return generatorError("Error: %s key not defined!" % cgKey)
  # sample app dirs
  for configModule in codegenContext:
    if "sampleApps" in configModule:
      for sa in configModule["sampleApps"]:
  ...
  for path in pathsToCheck:
    if not os.path.exists(path):
      return generatorError("Error: %s doesn't exist!" % path)
  return generatorSuccess("Sanity check successful!", [])


# Studio: Setup environment and generate
def setupGenerateStudio(context, verbose):
      ...
      for rcgm in sa["requiredCgm"]:
        for configModule in context["CODEGEN"]:
          ...
          for cgm in configModule["codeGenModules"]:
            ...
            try:
              tmpResult = genModule.generate(cgm, rcgm, context)
...
def generateContent(studioContext):
  ...
  # In case board is not stored, add it as a 'none' board
  if "ARCHITECTURE_ID" in studioContext:
    arch = studioContext["ARCHITECTURE_ID"].split("+")
    # There is only architecture and toolchain is stored in arch
    # Note: Second element is expected to be the toolchain in this case
    #       If this is not true, there is a bug here...
    if len(arch) == 2:
      arch.insert(1, "none")
      studioContext["ARCHITECTURE_ID"] = "+".join(arch)

  result = sanityCheckConfiguration(studioContext["CODEGEN"],
                                    studioContext["DEVICE_NAME"]["id"])
  if not result["success"]:
    return result
  return setupGenerateStudio(studioContext, True)
