...
static uint32_t get_cmd_id(volatile uint8_t* msg)
{
  uint32_t cmd_header = *((uint32_t *)msg);
  uint8_t* header = (uint8_t*)&cmd_header;
  //clearing the length field
  header[0] &= ~0x07;
  header[1] = 0;
  return cmd_header;
}

static void send_error_response(volatile uint8_t* cmd, errorcode_t err)
{
  struct gecko_cmd_packet* rsp = (struct gecko_cmd_packet*)gecko_rsp_msg_buf;
  rsp->header = get_cmd_id(cmd);
  uint8_t* payload = (uint8_t*)&rsp->data;
  payload[0] = bg_err_application_mismatched_or_insufficient_security & 0xff;
  payload[1] = ((bg_err_application_mismatched_or_insufficient_security & 0xff00) >> 8);
  ncp_sec_update_payload_length(rsp, 2);
}
...
void ncp_handle_command()
{
  ...
  struct gecko_cmd_packet * cmd;

  if (ncp_command_received()) {
    bool cmd_encrypted = ncp_sec_is_encrypted(rx_queue.data);

    if ((cmd_encrypted && ncp_sec_get_state() == ncp_sec_unencrypted)
        || !ncp_sec_validate_security(rx_queue.data)) {
      send_error_response(rx_queue.data, bg_err_application_mismatched_or_insufficient_security);
    } else {
      cmd = ncp_sec_decrypt_command((struct gecko_cmd_packet *)rx_queue.data);
      if (cmd == NULL) {
        gecko_external_signal(NCP_USART_SECURITY_SIGNAL);
        rx_queue_reset();
        return;
      } else {
        if (BGLIB_MSG_ID(cmd->header) == gecko_cmd_user_message_to_target_id) {
          // call user command handler:
          handle_user_command((uint8_t*)&cmd->data);
        } else {
          gecko_handle_command(cmd->header, (void*)&cmd->data);
        }
      }
    }

    // Symmetric encryption in command and response pair:
    rsp = ncp_sec_process_response((struct gecko_cmd_packet *)gecko_rsp_msg_buf, cmd_encrypted);
    if (rsp == NULL) {
      gecko_external_signal(NCP_USART_SECURITY_SIGNAL);
      rx_queue_reset();
      return;
...
bool ncp_transmit_enqueue(struct gecko_cmd_packet *evt)
{
  ...
  struct gecko_cmd_packet *msg = ncp_sec_process_event(evt);
  if (msg == NULL) {
    return false;
  }
  return ncp_enqueue(tx_queue, (uint8_t*)msg, BGLIB_MSG_LEN(msg->header) + BGLIB_MSG_HEADER_LEN, 0);
}
...
uint32_t ncp_transmit_queue_space()
{
  uint16_t available_size = tx_queue->size - NCP_TX_QUEUE_RESERVED_LEN;
  return (available_size - tx_queue->used) * NCP_BUF_SIZE;
}
