...
static bool ncp_enqueue(ncp_queue* queue, uint8_t* buf, uint32_t len);
...
void ncp_handle_command()
{
  if (ncp_command_received()) {
    ...
    ncp_transmit_enqueue(gecko_rsp_msg_buf);
...
bool ncp_transmit_enqueue(struct gecko_cmd_packet *evt)
{
  ...
  return ncp_enqueue(tx_queue, (uint8_t*)evt, BGLIB_MSG_LEN(evt->header) + BGLIB_MSG_HEADER_LEN);
}
...
static bool ncp_enqueue(ncp_queue* queue, uint8_t* buf, uint32_t len)
{
  ...
  if (len <= NCP_BUF_SIZE) {
    ...
    uint32_t space = (queue->size - queue->used) * NCP_BUF_SIZE;
    if (len > space) {
