...
def generate(cgm, rcgm, context):
  # Switch on/off verbose logging
  cg.setVerbose(cgm["verbose"])
  # Get framework version
  cgVer = cg.getVersion()
  cg.printLog("**** Using Code Generator Framework %s ****" % cgVer)
  if cgVer != "1.0.0":
  ...
  if isCompatible(cgm, context):
    ...
    if not context["INITIAL_GENERATION"]:
      ...
      for group in cgm["groups"]:
        ...
        if isCompatible(group, context):
          ...
          dstDir = os.path.join(context["PROJECT_INFO"]["rootDir"],
                                cgm["dstDir"])
          # copy files
          if group["command"] == "copy" and context["INITIAL_GENERATION"]:
            ...
            if "files" in group:
              for f in group["files"]:
                ...
                #         return cg.getResult()
          # link files
          elif group["command"] == "link" and context["INITIAL_GENERATION"]:
            cg.setupEnvironment(srcDir, dstDir)
            if "files" in group:
              for f in group["files"]:
                if os.path.isfile(os.path.join(srcDir, f)):
                  ...
                  if f.endswith(".h"):
                    if not cg.addIncludeDirectory(os.path.join(group["dstDir"], os.path.dirname(f))):
                      return cg.getResult()
          # add include path
          elif group["command"] == "include" and context["INITIAL_GENERATION"]:
            cg.setupEnvironment(srcDir, dstDir)
            if "incDirs" in group:
          ...
          # render templates
            ...
            if "imports" in group:
              ...
              impDirs = [os.path.join(os.path.dirname(__file__), os.path.dirname(f)) \
