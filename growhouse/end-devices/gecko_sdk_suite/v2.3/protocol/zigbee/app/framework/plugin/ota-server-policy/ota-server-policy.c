                                         ...
                                         uint32_t versionClientHas)
{
  ...
  /*
   * Check for upgrade or downgrade of firmware, independent of Version Policy
   */
  if ( (versionServerHas > versionClientHas) ||
        (versionServerHas < versionClientHas) )
  {
        return true;
  }
  return false;
...
uint8_t emberAfOtaServerImageBlockRequestCallback(EmberAfImageBlockRequestCallbackStruct* data)
{
  ...
  if (SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST
    ...
    // client treats the field. If we need to probe the client or don't have
    // a spot for it as an active OTA session, we tell it to WAIT_FOR_DATA
    ...
    if (emAfOtaServerDynamicBlockPeriodClientUsesSeconds(data->source)) {
