...
void emberAfPluginOtaClientStackStatusCallback(EmberStatus status)
{
  ...
  /* Added support to send OTA request immediately after device join the network */
  randomDelaySeconds = 1;
...
static void otaClientTick(void)
{
  ...
  switch (currentBootloadState) {
    ...
    case BOOTLOAD_STATE_DOWNLOAD:
      /* Check if short poll interval is 1sec then make it to 100ms while Downloading new image
       * Over the air to reduce the downloading time */
      if(emberAfGetShortPollIntervalMsCallback() > 500 )
         emberAfSetShortPollIntervalMsCallback(100);
      continueImageDownload();
      break;
    case BOOTLOAD_STATE_VERIFY_IMAGE:
      /* When image download is completed make short poll interval 1sec */
