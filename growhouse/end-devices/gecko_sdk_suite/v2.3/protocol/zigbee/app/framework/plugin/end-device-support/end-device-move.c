...
/*
 *	We need rejoin scan interval to 10 Seconds
 *	So, 10 second = 10 * 1000 ms
 */
#define MOVE_DELAY_MS (10 * 1000)
...
static void scheduleMoveEvent(void)
{
  ...
  if (EMBER_AF_REJOIN_ATTEMPTS_MAX == NEVER_STOP_ATTEMPTING_REJOIN
    ...
    emberAfAppPrintln("%s %d: %d", "Schedule move nwk",
                      networkIndex,
                      state->moveAttempts);
    emberAfNetworkEventControlSetDelayMS(emberAfPluginEndDeviceSupportMoveNetworkEventControls,
    ...
    emberAfAppPrintln("Rebooting Soil node");
    emberAfAppFlush();
    /* Reboot the board If we reached Max Limit(EMBER_AF_REJOIN_ATTEMPTS_MAX) for network rejoin
       Chances of rejoin is good after board reboot.
       After reboot there is no such impact on netwok and binding table also remain same.
       So, After reboot if end node found proper parent network then it will join the network
       and it will start reporting data
     */
    halReboot();
  }
}
...
void emberAfPluginEndDeviceSupportMoveNetworkEventHandler(void)
{
  ...
  /* First Try secure rejoin */
  if(option == true)
  {
	  secure = true;
	  option = false;
  }
  /* Second Try un-secure rejoin */
  else
  {
	  secure = false;
	  option = true;
  ...
  emberAfCorePrintln("Is secure rejoin : %d", secure);
