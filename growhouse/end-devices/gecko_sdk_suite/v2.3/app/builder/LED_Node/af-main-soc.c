...
void emberAfMainInit(void)
{
  ...
  emberSerialInit(HAL_SERIAL_PORT_USART2, BAUD_RATE, PARITY_NONE, 1);

  /* Make Zigbee presence detection GPIO as LOW */
  // configure GPIO pins
  GPIO_PinModeSet(HAL_ZIGBEE_CONN_PORT, HAL_ZIGBEE_CONN_PIN, gpioModePushPull, 0);
...
int emberAfMain(MAIN_FUNCTION_PARAMETERS)
{
  ...
  while (true) {
    ...
    /* Process the BLE USART2 request */
    BLEUartProcessInput();
