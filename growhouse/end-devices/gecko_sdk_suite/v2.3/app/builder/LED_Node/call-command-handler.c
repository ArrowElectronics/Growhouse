...
/* extern for Global structure to store the LED intensity state of all
 * LED channel 1 to 6.
 */
    ...
    case ZCL_MOVE_TO_LEVEL_COMMAND_ID:
      {
        ...
        wasHandled = emberAfLevelControlClusterMoveToLevelCallback(level,
                                                                   ...
                                                                   optionOverride);
#ifdef EMBER_AF_PLUGIN_DPOT_AD5206
        emberAfCorePrintln("Endpoint number : %d , level : %d",cmd->apsFrame->destinationEndpoint, level);
        wasHandled = halAD5204DriverSetLedLevel(cmd->apsFrame->destinationEndpoint, level);

        /* store the updated level of the respective LED Channel */
        Current_LED_Configuration.LED_Channel_intensity[(cmd->apsFrame->destinationEndpoint) - 1] = level;
        /* TDOD: write the updated value in flash */
